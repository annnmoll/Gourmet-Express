...
exports.createUser = async (req, res) => {
    try {
        ...
        const { name, email, password, location , confirmPassword  , otp} = req.body;
         ...
         if(otp != otpResponse.otp) { return res.status(400).json({
            success : false ,
            message : 'Invalid otp'
         })}
        ...
        const response = await User.create({
            name, location, email, password: hashedpass
        }) ; 
...
exports.getUser = async (req, res) => {
    try {
       ...
       if(isValid){
        ...
        const payload = { email: user.email, id: user._id, location: user.location } 
